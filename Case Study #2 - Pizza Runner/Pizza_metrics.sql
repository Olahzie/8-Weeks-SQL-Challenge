...
SELECT
	...
	COUNT(DISTINCT temp_customer_orders.order_id) AS orders
...
SELECT 
	customer_id,
	pizza_name,
    COUNT(temp_customer_orders.pizza_id)
FROM temp_customer_orders
...
SELECT
	customer_id,
	COUNT(temp_customer_orders.order_id)  filter (where exclusions != '' OR extras != '') as with_changes,
	 COUNT(temp_customer_orders.order_id)  filter (where exclusions = '' AND extras = '') as without_changes 
FROM temp_customer_orders
...
SELECT EXTRACT(ISODOW FROM order_time) AS Days, 
	   COUNT(order_id) AS orders
FROM temp_customer_orders
GROUP BY Days;